...
from protocol import (
    CHAR_TX_UUID, CHAR_RX_UUID, DEVICE_PREFIX,
    BLEFlags, ServiceCommand, build_message, parse_response, BLEResponse,
    parse_port_statistics, PortInfo
)
...
class BLEManager:
    ...
    def __init__(self, log_callback: Optional[Callable[[str], None]] = None):
        ...
        self._scanned_devices: Dict[str, BLEDevice] = {}  # 缓存扫描到的设备对象
        
        # 数据缓存
        self.port_states = {}  # 缓存端口状态
    ...
    async def scan_devices(self, timeout: float = 5.0) -> List[DeviceInfo]:
        ...
        for address, (device, adv_data) in devices.items():
            if device.name and device.name.startswith(DEVICE_PREFIX):
                self._scanned_devices[device.address] = device  # 缓存设备对象
                info = DeviceInfo(
                    name=device.name,
    ...
    async def _polling_loop(self):
        ...
        while self._polling_active and self.connected:
            try:
                # 1. 获取所有端口的基础状态 (电压电流功率)
                stats = await self.get_port_status()
                if stats:
                    for s in stats:
                        if s.port_id not in self.port_states:
                            self.port_states[s.port_id] = {}
                        self.port_states[s.port_id].update({
                            "state": s.status,
                            "protocol": s.protocol,
                            "voltage": s.voltage,
                            "current": s.current,
                ...
                # 2. 轮询端口详细信息 (PD状态)
                # 为了避免阻塞，每次循环只更新一个端口的PD信息？
                # 或者全量更新但间隔长一点？
                # 这里选择全量更新，但间隔1.5秒
                for port_id in range(5):
                    # 只有当端口有电流或者还没有PD信息时才去查询，节省带宽
                    # 但为了检测线材插入，还是需要查询
                    resp = await self.execute(ServiceCommand.GET_PORT_PD_STATUS, bytes([port_id]), timeout=1.0)
                    if resp and resp.payload:
                        if port_id not in self.port_states:
                            self.port_states[port_id] = {}
                        self.port_states[port_id]['pd_raw'] = resp.payload
    ...
    def _on_disconnect(self, client):
        ...
        self.log(f"设备已断开连接: {client.address if client else 'Unknown'}")
        self._connected = False
        self.response_event.set()  # Unblock any waiting commands
    ...
    async def connect(self, address: str) -> bool:
        ...
        try:
            ...
            if device:
                self.log(f"使用缓存设备对象: {device.name}")
                self.device = device
                self.client = BleakClient(device, disconnected_callback=self._on_disconnect)
            else:
                self.log("未找到缓存设备，尝试直接连接地址...")
                # 创建一个临时的 DeviceInfo 兼容对象
                self.device = type('DeviceInfo', (), {'name': 'Unknown', 'address': address})()
            ...
            self._reconnect_attempts = 0
            self.log(f"已连接到 {self.device.name}")
